...
const refreshSeatIndicator = () => {
  ...
  for (let i = 0; i < selectedSeats.length; i++) {
    seat = document.getElementById(selectedSeats[i])
...
window.onload = () => {
  if (localStorage.getItem('maxSeats')) {
    lastMaxSeats = JSON.parse(localStorage.getItem('maxSeats'));

    if (lastMaxSeats != maxSeats) {
      localStorage.setItem('maxSeats', JSON.stringify(maxSeats));
    } else {
